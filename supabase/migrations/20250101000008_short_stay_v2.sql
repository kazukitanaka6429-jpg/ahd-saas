...
-- Phase 7: Short Stay Re-architecture
...
CREATE POLICY "Enable all for authenticated users based on facility_id" ON short_stay_records
    FOR ALL
    TO authenticated
    USING (facility_id = (select facility_id from staffs where auth.uid() = staffs.user_id))
    WITH CHECK (facility_id = (select facility_id from staffs where auth.uid() = staffs.user_id));
