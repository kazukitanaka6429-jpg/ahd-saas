...
export async function upsertStaff(formData: FormData) {
    try {
        ...
        if (error) {
            logger.error('Error upserting staff:', error)
            return { error: '職員の保存に失敗しました' }
        }

        revalidatePath('/staffs')
        return { success: true }
    } catch (e: any) {
        logger.error('Unexpected error in upsertStaff', e)
...
export async function createInvitation(formData: FormData) {
    try {
        ...
        logger.error('Unexpected error in createInvitation', e)
        return { error: '予期せぬエラーが発生しました' }
    }
}
