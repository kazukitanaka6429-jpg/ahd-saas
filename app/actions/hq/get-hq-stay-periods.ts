...
export async function getHqStayPeriods(year: number, month: number, facilityIdArg?: string): Promise<ResidentStayData[]> {
    try {
        await protect()

        const staff = await getCurrentStaff()
        if (!staff) {
            logger.warn('Unauthorized access to getHqStayPeriods')
            return []
        }
        ...
        if (staff.role === 'admin') {
            if (facilityIdArg) {
                ...
                // Auto-select first facility for Admin if not provided (or fallback to assigned if we want, but sticking to pattern)
                // If default behavior is "show assigned if exists", we can enable that.
                // But consistently, if I am admin, I might want to see the "First" facility if no ID is in URL to be safe,
                // OR see my assigned one.
                // Current pattern in `getHqDailyData` overwrites it.
                // Let's copy `getHqDailyData` logic for consistency.
        ...
        } else if (facilityIdArg && facilityIdArg !== staff.facility_id) {
            // Non-admin trying to access other facility
            logger.warn('Unauthorized facility access attempt by staff', {
                staffId: staff.id,
                targetFacility: facilityIdArg
            })
            // Ignore arg, use assigned
        }
        ...
        logger.error('Unexpected error in getHqStayPeriods', e)
        return []
    }
}
